...
def fetch_gemini_models(api_key):
    """Fetch available Gemini generate-capable model names for a given API key."""
    if not api_key:
        return [], "Enter a Gemini API key to load models."
    ...
    try:
        ...
        if not unique_names:
            return [], "No generate-capable Gemini models were returned for this key."
        return unique_names, None
    except Exception as e:
        return [], f"Could not load Gemini models: {e}"
...
with st.sidebar:
    ...
    available_models, models_error = fetch_gemini_models(gemini_api_key) if gemini_api_key else ([], None)
    selected_model_name = st.selectbox(
        "Gemini Model",
        options=available_models if available_models else ["No models loaded"],
        index=0,
        disabled=not bool(available_models),
        help="Models are loaded from your Gemini account on app refresh."
    )
    if models_error:
    ...
    run_enrichment = st.button(
        "Run Enrichment",
        type="primary",
        disabled=not (uploaded and gemini_api_key and available_models)
    )
...
# Run Enrichment
# -------------------------------------------------
if run_enrichment and gemini_api_key and available_models:
